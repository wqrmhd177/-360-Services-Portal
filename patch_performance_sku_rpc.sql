-- Performance: single-pass SKU summary RPC (no duplicate MV scan).
-- Run in Supabase SQL Editor after setup_sku_performance_mv.sql.
...
BEGIN
  ...
  SELECT mv_refreshed_at INTO v_mv_refreshed
  FROM ops_sku_daily_performance
  ORDER BY mv_refreshed_at DESC NULLS LAST
  LIMIT 1;
  ...
  EXECUTE format(
    ...
    SELECT
      COALESCE((SELECT MAX(total_records) FROM paged), 0),
      COALESCE((
        SELECT jsonb_agg(
          jsonb_build_object(
            'sku', sku,
            ...
            'dispatch_to_delivery_pct', dispatch_to_delivery_pct,
