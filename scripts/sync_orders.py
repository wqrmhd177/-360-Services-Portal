...
ON CONFLICT (order_id, sku) DO UPDATE SET
  ...
  courier_tracking_id = EXCLUDED.courier_tracking_id,
  confirmation_pending_date = EXCLUDED.confirmation_pending_date,
        ...
        rows.append(
            {
                ...
                "courier_tracking_id": metabase_str(
                    r,
                    "Courier_tracking_id",
                    "courier_tracking_id",
                ...
                "confirmation_pending_date": parse_date(r.get("Confirmation_Pending_Date")),
...
def row_to_tuple(row: dict[str, Any], synced_at: datetime) -> tuple[Any, ...]:
    return (
        ...
        row.get("courier_tracking_id") or "",
        row.get("confirmation_pending_date"),
        row.get("platform") or "",
        row.get("order_date"),
...
def upsert_postgres_copy(rows: list[dict[str, Any]], synced_at: datetime) -> None:
    ...
    SELECT
      ...
      delivery_partner,
      courier_tracking_id,
      NULLIF(confirmation_pending_date, '')::TIMESTAMPTZ,
      platform,
...
def upsert_rest(rows: list[dict[str, Any]], synced_at: datetime, batch_size: int, workers: int) -> None:
    ...
    def payload(row: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "courier_tracking_id": row.get("courier_tracking_id"),
            "confirmation_pending_date": row["confirmation_pending_date"].isoformat() if row.get("confirmation_pending_date") else None,
