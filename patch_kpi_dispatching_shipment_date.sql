-- KPI: Dispatching ages from shipment_date_log; unique order_id at day/country/total levels.
-- Run AFTER patch_kpi_returning_status.sql (step 15).
...
BEGIN
  v_statuses := CASE p_group_id
    ...
    WHEN 'return'              THEN ARRAY['Return in Transit']
    ELSE NULL
  END;
  ...
  v_days_from_label := CASE p_group_id
    ...
    WHEN 'dispatching'         THEN 'shipmentDateLog'
    WHEN 'shipped'             THEN 'shipmentDateLog'
    WHEN 'undelivered'         THEN 'undeliveredDate'
    WHEN 'return'              THEN 'finalActionDateUndelivered'
    ELSE 'orderDate'
  END;
  ...
  v_title := CASE p_group_id
    ...
    WHEN 'return'               THEN 'Orders in Returning'
    ELSE p_group_id
  END;
  ...
  source AS (
      ...
      CASE
        ...
        WHEN p_group_id = 'undelivered' THEN
          CASE WHEN p.undelivered_date IS NOT NULL
            ...
            ELSE NULL
          END
        WHEN p_group_id = 'return' THEN
          CASE WHEN p.final_action_date_undelivered IS NOT NULL
            THEN GREATEST(0, v_today - (p.final_action_date_undelivered AT TIME ZONE 'America/Los_Angeles')::DATE)
            ELSE NULL
          END
        ELSE
    ...
    FROM (
      ...
      SELECT
        ...
        pr.final_action_date_undelivered,
  ...
  country_rows AS (
    SELECT
      ...
      jsonb_agg(
        jsonb_build_object(
          'label', sr.subgroup_label,
          'orders', sr.orders,
          'orderIds', to_jsonb(sr.order_ids),
          'orderGroups', sr.order_groups
        )
        ORDER BY sr.orders DESC, sr.subgroup_label
      ) AS subgroups
    FROM subgroup_rows sr
    JOIN (
      SELECT days, country, COUNT(DISTINCT order_id)::INTEGER AS orders
      FROM source
      GROUP BY days, country
    ) dc ON dc.days IS NOT DISTINCT FROM sr.days AND dc.country = sr.country
    GROUP BY sr.days, sr.country, dc.orders
  ),
  day_rows AS (
    SELECT
      cr.days,
      ...
      jsonb_agg(
        jsonb_build_object(
          'country', cr.country,
          'orders', cr.orders,
          'subgroups', cr.subgroups
        )
        ORDER BY cr.orders DESC, cr.country
      ) AS countries
    FROM country_rows cr
    JOIN (
      SELECT days, COUNT(DISTINCT order_id)::INTEGER AS orders
      FROM source
      GROUP BY days
    ) dd ON dd.days IS NOT DISTINCT FROM cr.days
    GROUP BY cr.days, dd.orders
  ),
  country_bifurcation AS (
    SELECT
      country,
  ...
  SELECT
    COALESCE((SELECT COUNT(DISTINCT order_id)::INTEGER FROM source), 0),
    COALESCE(jsonb_agg(
      jsonb_build_object(
        'days',  days,
        'label', CASE
...
COMMENT ON FUNCTION get_ops_orders_status_detail IS
  'KPI drill-down: dispatching ages from shipment_date_log; unique order_id at day/country/total.';
