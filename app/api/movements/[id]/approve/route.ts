...
import { approveMovement } from "@/lib/movements/db";
...
export async function POST(
  ...
  try {
    ...
    const movement = await approveMovement(session!, id, remarks);
    return NextResponse.json(movement);
  } catch (err) {
    const msg = err instanceof Error ? err.message : "Failed to approve movement";
