...
import { syncOpPerformanceFromSheet } from "@/lib/operations/syncOpPerformance";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await syncOpPerformanceFromSheet();
    if (!result.ok) {
      return NextResponse.json(
        {
          error: result.error ?? "OP sync failed",
          hint: "Run setup_ops_op_performance.sql, then Sync Data again.",
        },
        { status: 502 },
      );
    }
