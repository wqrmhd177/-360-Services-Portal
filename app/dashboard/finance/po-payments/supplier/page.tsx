...
import {
  errorMessages,
  getAllPos,
  isSupplierPaymentEligibleStatus,
  markSupplierPaymentPaidWithProofUrl,
  revertSupplierPaymentToUnpaid,
...
function buildHref(view: ViewFilter, search: string) {
  ...
  const s = qs.toString();
  return s ? `/dashboard/finance/po-payments/supplier?${s}` : "/dashboard/finance/po-payments/supplier";
}

export default async function SupplierPoPaymentsPage({
  searchParams,
}: {
  ...
  async function savePaidAction(formData: FormData) {
    "use server";
    await markSupplierPaymentPaidWithProofUrl(formData, "/dashboard/finance/po-payments/supplier");
  }

  async function revertAction(formData: FormData) {
    "use server";
    await revertSupplierPaymentToUnpaid(formData, "/dashboard/finance/po-payments/supplier");
  }

  async function deleteProofAction(formData: FormData) {
    "use server";
    await deleteSupplierPaymentProof(formData, "/dashboard/finance/po-payments/supplier");
  }

  const pos = await getAllPos();

  const eligible = pos.filter((po) => isSupplierPaymentEligibleStatus(po.status));
  ...
  const viewFiltered = filtered.filter((po) => {
    ...
    return po.supplier_payment_status === view;
  });

  const unpaidCount = filtered.filter((p) => p.supplier_payment_status === "unpaid").length;
  const paidCount = filtered.filter((p) => p.supplier_payment_status === "paid").length;

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight text-gray-900">Supplier Payments</h2>
          <p className="text-sm text-gray-500">
            POs become due when status reaches Shipment Received At LMP Warehouse.
          </p>
        </div>
      ...
      <div className="card">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <form method="GET" action="/dashboard/finance/po-payments/supplier" className="flex items-center gap-2">
            ...
            <table className="min-w-full text-xs">
              <thead className="border-b border-gray-200 text-gray-500">
                <tr>
                  ...
                  <th className="px-2 py-2 font-medium text-center align-middle">Supplier</th>
                  <th className="px-2 py-2 font-medium text-center align-middle">Status</th>
                  <th className="px-2 py-2 font-medium text-center align-middle">Supplier Payment</th>
                  ...
                  return (
                    <tr key={po.id} className="border-b border-gray-100 last:border-0">
                      ...
                      <td className="px-2 py-2 text-center align-middle text-gray-700">
                        ...
                        <div className="text-[10px] text-gray-500">{po.supplier_location}</div>
                      </td>
                      ...
                      <td className="px-2 py-2 text-center align-middle">
                        <div className="flex items-center justify-center gap-1.5 flex-wrap">
                          ...
                          <PoPaymentActions
                            ...
                            currentStatus={po.supplier_payment_status}
                            currentProofUrl={po.supplier_payment_proof}
