...
export default function ProductAvailabilityPage() {
  ...
  const paginatedDisplayed = displayedRequests.slice(
    (paCurrentPage - 1) * PA_ITEMS,
    paCurrentPage * PA_ITEMS
  );
  ...
  const handleCheckInventory = async () => {
    ...
    try {
      ...
      console.error(err);
      setInventoryError(
        "Unable to fetch inventory right now. You can still submit this request."
      );
  ...
  const handleCreateRequest = async (e: React.FormEvent) => {
    ...
    if (!createForm.market) { setError("Please select a market."); return; }
    ...
    if (isXlsx) {
      ...
      reader.onload = async (ev) => {
        ...
        const sheet = workbook.worksheets.find(
          (ws) => ws.name !== "_Options" && ws.name !== "Valid Options"
        );
        if (!sheet) return;
        const rows: string[][] = [];
        sheet.eachRow((row) => {
  ...
  const handleDownloadTemplate = async () => {
    ...
    optSheet.addRow([
      "market",
      "UAE  |  KSA  |  PAK  |  QTR  |  KWT  |  OMN  |  BHR  |  IRQ  |  USA  (one market per request)",
    ]);
  ...
  const handleSubmitResponse = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await submitProductAvailabilityResponse({
        ...
        singleUnitPrice: isNotAvailable
          ? null
    ...
    <div className="max-w-7xl mx-auto space-y-6 p-4 sm:p-6">
      <div>
        ...
        <p className="text-sm text-gray-600 mt-1">
          Create and track market availability requests.
        </p>
      </div>
      ...
      <div
        className={`grid grid-cols-3 ${
          canCreate ? "sm:grid-cols-4 md:grid-cols-8" : "sm:grid-cols-3 md:grid-cols-6"
        } gap-2`}
      >
        {canCreate && (
          <button
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex rounded-lg border border-gray-200 overflow-hidden text-sm">
              ...
              <button
                ...
                className={`px-4 py-1.5 border-l border-gray-200 ${
                  newTabMode === "bulk"
                    ? "bg-blue-600 text-white"
                    : "bg-white text-gray-600 hover:bg-gray-50"
                }`}
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span className="text-xs text-gray-500">
                  Fill in the template — dropdowns available for product_status and priority_level.
                  Upload the filled file (.xlsx or .csv) below.
                </span>
              </div>
              ...
              <div className="rounded-lg border border-blue-100 bg-blue-50 p-3 text-xs space-y-1.5">
                ...
                <div className="text-blue-900">
                  ...
                  {["UAE", "KSA", "PAK", "QTR", "KWT", "OMN", "BHR", "IRQ", "USA"].map((m) => (
                    <span key={m} className="font-mono bg-white/70 px-1 rounded mr-1">
                      {m}
                    </span>
                  ))}
                ...
                <div className="text-blue-900">
                  ...
                  {["already_listed", "not_listed", "not_sure"].map((s) => (
                    <span key={s} className="font-mono bg-white/70 px-1 rounded mr-1">
                      {s}
                    </span>
                  ))}
                </div>
                <div className="text-blue-900">
                  ...
                  <span className="font-mono bg-white/70 px-1 rounded mr-1">urgent</span>
                ...
                <div className="overflow-x-auto rounded-lg border border-gray-200">
                  <table className="min-w-full text-xs">
                    <thead className="bg-gray-50">
                      <tr>
                        {["#", "Product", "Reseller", "Market", "SKU", "Status", "Priority", "Issues"].map(
                          (h) => (
              ...
              {bulkImportResult && (
                <div className="p-3 rounded-lg bg-green-50 text-green-700 text-sm">
                  {bulkImportResult.successCount} draft
                  {bulkImportResult.successCount !== 1 ? "s" : ""} created.
                  {bulkImportResult.failedRows.length > 0 &&
                    ` Rows failed: ${bulkImportResult.failedRows.join(", ")}.`}{" "}
              ...
              {bulkCsvRows.length > 0 && bulkCsvRows.some((r) => r.errors.length === 0) && (
                ...
                >
                  ...
                  {bulkImporting
                    ? "Importing…"
                    : `Import ${bulkCsvRows.filter((r) => r.errors.length === 0).length} Valid Row(s) as Drafts`}
                </button>
              )}
            </div>
          )}
            ...
            <form onSubmit={handleCreateRequest} className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setCreateForm((p) => ({
                        ...p,
                        productStatus: e.target.value as CreateFormState["productStatus"],
                      }))
                    }
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setCreateForm((p) => ({
                        ...p,
                        priorityLevel: e.target.value as CreateFormState["priorityLevel"],
                      }))
                    }
                    className="w-full border rounded-lg px-3 py-2"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input
                  ...
                  value={createForm.resellerName}
                  onChange={(e) =>
                    setCreateForm((p) => ({ ...p, resellerName: e.target.value }))
                  }
                />
                <input
                  className="border rounded-lg px-3 py-2"
                  ...
                  onChange={(e) =>
                    setCreateForm((p) => ({ ...p, productName: e.target.value }))
                  }
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input
                  className="border rounded-lg px-3 py-2"
                  placeholder={
                    createForm.productStatus === "already_listed" ? "SKU *" : "SKU (optional)"
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCreateForm((p) => ({ ...p, referenceLink: e.target.value }))
                  }
                />
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                {inventoryError && (
                  <span className="text-sm text-amber-600">{inventoryError}</span>
                )}
              </div>
                ...
                <div className="border rounded-lg p-3 bg-gray-50">
                  <p className="text-sm text-gray-700 mb-2">
                    ...
                    <span className="font-semibold">
                      {inventoryLookup.matchedPrefix || "N/A"}
                    </span>{" "}
                    | Results:{" "}
                  ...
                  <div className="space-y-2 max-h-56 overflow-auto">
                    {inventoryLookup.warehouseGroups.map((group) => (
                      <div key={group.warehouseName} className="bg-white border rounded-md p-2">
                        ...
                        >
                          <span>
                            {group.warehouseName} (Qty: {group.totalQuantity})
                          </span>
              ...
              <div>
                ...
                <input
                  ...
                  accept="image/*"
                  multiple
                  onChange={(e) => handleCreateImageChange(e.target.files)}
                />
                <div className="text-xs text-gray-500 mt-1">
                  {createForm.images.length}/5 selected
                </div>
              </div>
        ...
        <div className="rounded-xl overflow-hidden border border-amber-200 bg-amber-50 p-4 space-y-3">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold text-amber-700">
              Drafts — Add Photos to Submit
            </span>
            <span className="text-xs text-amber-600">
              ({draftRequests.length} pending photo upload)
            </span>
          </div>
          {draftRequests.length === 0 && (
            <p className="text-sm text-amber-600">
              No drafts yet. Use Bulk Upload in the New tab to create drafts.
            </p>
          )}
              ...
              <div className="flex items-start justify-between gap-2 flex-wrap">
                <div>
                  <div className="font-medium text-gray-900">
                    {titleCaseWords(req.product_name)}
                  </div>
                  <div className="text-xs text-gray-500">
                    {req.reseller_name} · {req.market || req.markets?.join(", ")} ·{" "}
                    {req.priority_level === "urgent" ? "Urgent" : "Normal"}
                    {req.sku && ` · SKU: ${req.sku}`}
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() =>
                    setDraftPhotoOpen(draftPhotoOpen === req.id ? null : req.id)
                  }
                  ...
                  <div>
                    ...
                    {draftPhotoFiles[req.id]?.length ? (
                      <p className="text-xs text-gray-500 mt-1">
                        {draftPhotoFiles[req.id].length}/5 selected
                      </p>
                    ) : null}
                  </div>
                  <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <select
                  ...
                  onChange={(e) =>
                    setResponseForm((p) => ({
                      ...p,
                      availability: e.target.value as PurchaserResponseState["availability"],
                    }))
                  }
                  className="border rounded-lg px-3 py-2 w-full"
                >
                ...
                {responseForm.availability !== "not_available" && (
                  <select
                    ...
                    onChange={(e) =>
                      setResponseForm((p) => ({
                        ...p,
                        stockStatus: e.target.value as PurchaserResponseState["stockStatus"],
                      }))
                    }
                    className="border rounded-lg px-3 py-2 w-full"
                  >
                ...
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <input
                      ...
                      onChange={(e) =>
                        setResponseForm((p) => ({ ...p, singleUnitPrice: e.target.value }))
                      }
                    />
                    {responseForm.stockStatus === "bulk_limited_both" && (
                      <input
                        ...
                        onChange={(e) =>
                          setResponseForm((p) => ({ ...p, bulkUnitPrice: e.target.value }))
                        }
                      />
                    )}
                  </div>
                  <textarea
                    ...
                    onChange={(e) =>
                      setResponseForm((p) => ({ ...p, remarks: e.target.value }))
                    }
                  />
                  <div>
                    ...
                    <input
                      ...
                      accept="image/*"
                      multiple
                      onChange={(e) => handleResponseImageChange(e.target.files)}
                      className="block text-sm"
                    />
                    <div className="text-xs text-gray-500 mt-1">
                      {responseForm.images.length}/5 selected
                    </div>
                  </div>
                </>
              )}
      ...
      {filter !== "new" &&
        ...
        (() => {
          if (displayedRequests.length === 0) {
            return (
              <div
                className="rounded-xl border border-white/10 px-4 py-10 text-center text-white/60 text-sm"
                style={{
                  background:
          ...
          return (
            <>
              ...
              {paTotalPages > 1 && (
                <div
                  className="flex items-center justify-between px-3 py-2 rounded-xl border border-white/10 mb-2"
                  style={{
                    background:
                      "linear-gradient(135deg, #0f0f23 0%, #1a1a2e 35%, #1e1b4b 70%, #2d1b69 100%)",
                  }}
                >
                  <span className="text-xs text-white/60">
                    Showing {(paCurrentPage - 1) * PA_ITEMS + 1}–
                    {Math.min(paCurrentPage * PA_ITEMS, displayedRequests.length)} of{" "}
                    {displayedRequests.length} requests
                  </span>
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() => setPaCurrentPage(1)}
                      disabled={paCurrentPage === 1}
                      className="px-2 py-1 rounded text-xs text-white/70 hover:text-white hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed"
                    >
                      « First
                    </button>
                    <button
                      onClick={() => setPaCurrentPage((p) => Math.max(1, p - 1))}
                      disabled={paCurrentPage === 1}
                      className="px-2 py-1 rounded text-xs text-white/70 hover:text-white hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed"
                    >
                      ‹ Prev
                    ...
                    <button
                      onClick={() => setPaCurrentPage((p) => Math.min(paTotalPages, p + 1))}
                    ...
                    <button
                      onClick={() => setPaCurrentPage(paTotalPages)}
                      disabled={paCurrentPage >= paTotalPages}
                      className="px-2 py-1 rounded text-xs text-white/70 hover:text-white hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed"
                    >
                      Last »
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2 flex-wrap">
                            <span className="font-semibold text-white text-sm leading-tight">
                              {titleCaseWords(request.product_name)}
                            </span>
                            <span
                              className={`px-2 py-0.5 rounded-full text-xs shrink-0 ${statusClass(request.derived_status)}`}
                            >
                          ...
                          {request.reference_link && (
                            <div className="text-xs text-blue-400 truncate">
                              <a href={request.reference_link} target="_blank" rel="noreferrer">
                                {request.reference_link}
                              </a>
                            </div>
                          )}
                          {request.remarks && (
                            <div className="text-xs text-white/50 italic">{request.remarks}</div>
                          )}
                        </div>
                      </div>
                      ...
                      {!isPurchaser && (
                        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-white/70">
                          <span>
                            <span className="text-white/40">Reseller:</span>{" "}
                            {titleCaseWords(request.reseller_name)}
                          </span>
                          <span>
                            <span className="text-white/40">Market:</span>{" "}
                            {request.market || request.markets?.join(", ")}
                          </span>
                        </div>
                      )}
                      {noMapping && (
                        <div className="text-xs text-amber-500">
                          No purchaser mapped for {request.market || "this market"}
                        </div>
                      )}
                      ...
                      <div className="flex items-center justify-between pt-1 border-t border-white/10">
                        ...
                        <div className="flex items-center gap-2 flex-wrap justify-end">
                          {isPurchaser &&
                            (canRespond ? (
                              <button
                                onClick={() => openRespondForm(request.id)}
                                className="px-4 py-1.5 rounded-lg bg-blue-600 text-white text-xs font-medium"
                              >
                                Respond
                              </button>
                            ) : (
                          ...
                          {(canCreate || isManager) && request.derived_status === "completed" && (
                            <button
                              ...
                              className="px-4 py-1.5 rounded-lg bg-violet-600 text-white text-xs font-medium"
                            >
                              See Feedback
                            </button>
                          )}
                          {!isPurchaser &&
                            ...
                            request.derived_status !== "cancelled" &&
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full text-sm">
                    <thead className="bg-white/5">
                      <tr>
                        <th className="px-3 py-3 text-left text-xs font-semibold text-white/70">
                          Request No.
                        </th>
                        <th className="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-white/70">
                          Product
                        </th>
                        {!isPurchaser && (
                          <th className="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-white/70">
                      ...
                      {paginatedDisplayed.map((request) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-3 py-2">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  {request.remarks && (
                                    <div className="text-xs text-white/55 italic mt-0.5 max-w-[240px]">
                                      {request.remarks}
                                    </div>
                                  )}
                                </div>
                            ...
                            <td className="px-3 py-2">
                              <span
                                className={`px-2 py-1 rounded-full text-xs ${statusClass(request.derived_status)}`}
                              >
                                {formatDerivedStatusLabel(request.derived_status)}
                              </span>
                            </td>
                              ...
                              <td className="px-3 py-2">
                                ...
                                request.derived_status !== "cancelled" ? (
                                  cancelConfirmId === request.id ? (
                                    ...
                                    <button
                                      onClick={() => setCancelConfirmId(request.id)}
                                      className="px-3 py-1.5 rounded-md bg-gray-600 text-white text-xs"
                                    >
                                      Cancel
                                    </button>
                                  )
          ...
          >
            ...
            <div className="p-4 space-y-4">
              <div className="text-white font-medium">
                {selectedFeedbackRequest.product_name}
              </div>
              <div className="text-xs text-white/50">
                Market:{" "}
                {selectedFeedbackRequest.market || selectedFeedbackRequest.markets?.join(", ")}
                {selectedFeedbackRequest.remarks && (
                  <span className="ml-3 italic">
                    Remarks: {selectedFeedbackRequest.remarks}
                  </span>
                )}
              </div>
              ...
              <div className="border border-white/10 rounded-lg p-3 bg-white/5">
                <div className="text-xs font-semibold text-white/50 uppercase mb-2">
                  ...
                  {selectedFeedbackRequest.response &&
                    ` — Round ${selectedFeedbackRequest.response.round_number}`}
                </div>
                {!selectedFeedbackRequest.response ? (
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="text-white/90">
                      ...
                      <span className="text-white">
                        {formatAvailabilityLabel(selectedFeedbackRequest.response.availability)}
                      </span>
                    </div>
                    <div className="text-white/90">
                      Stock Status:{" "}
                      <span className="text-white">
                        {formatStockStatusLabel(selectedFeedbackRequest.response.stock_status)}
                      </span>
                    </div>
                    <div className="text-white/90">
                      Single Unit Price:{" "}
                      <span className="text-white">
                        {selectedFeedbackRequest.response.single_unit_price ?? "N/A"}
                      </span>
                    </div>
                    <div className="text-white/90">
                      Bulk Unit Price:{" "}
                      <span className="text-white">
                        {selectedFeedbackRequest.response.bulk_unit_price ?? "N/A"}
                      </span>
                    </div>
                    <div className="text-white/90">
                      Remarks:{" "}
                      <span className="text-white">
                        {selectedFeedbackRequest.response.remarks || "N/A"}
                      </span>
                    </div>
                    <div className="text-white/40 text-xs">
                      {new Date(selectedFeedbackRequest.response.created_at).toLocaleString()}
                    </div>
                  </div>
                )}
              </div>
                    ...
                    {selectedFeedbackRequest.responseHistory.slice(1).map((r) => (
                      <div key={r.id} className="px-3 py-2 text-xs space-y-1">
                        ...
                        <div className="text-white/70">
                          Availability: {formatAvailabilityLabel(r.availability)}
                        </div>
                        {r.remarks && (
                          <div className="text-white/60 italic">Remarks: {r.remarks}</div>
                        )}
                      </div>
                    ))}
                  </div>
              ...
              {/* Alternative search (agents only, when latest response is not_available) */}
              {isAgent &&
                selectedFeedbackRequest.response?.availability === "not_available" &&
                selectedFeedbackRequest.assignment_status !== "pending" && (
                  <div className="border border-amber-500/30 rounded-lg p-3 bg-amber-500/10 space-y-3">
                    ...
                    <p className="text-xs text-amber-200/70">
                      Re-send this request to the purchaser with updated remarks. The purchaser
                      will see the previous response history and your new notes.
                    </p>
                    {!showAltSearch ? (
                      <button
                        type="button"
                        onClick={() => {
                          ...
                          setAltSearchRemarks(selectedFeedbackRequest.remarks || "");
                        }}
                      ...
                      <div className="space-y-2">
                        {altSearchError && (
                          <div className="text-xs text-red-400">{altSearchError}</div>
                        )}
                        <textarea
                          rows={3}
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            onClick={async () => {
                              if (!altSearchRemarks.trim()) {
                                setAltSearchError("Please add updated remarks.");
                                return;
                              }
