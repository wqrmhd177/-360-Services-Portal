...
export default function ApproverPurchaseOrdersPage() {
  ...
  async function loadPos() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Extract and fetch user names
        const emails: string[] = [];
        data?.forEach((po: Po) => {
          const email = po.created_by_email;
  ...
  function handleSelectAll() {
    if (selectedIds.size === filteredPos.length) {
      ...
      setSelectedIds(new Set(filteredPos.map((po: Po) => po.id)));
    }
  }
  ...
  function downloadCSV() {
    ...
    const rows = selectedPos.map((po) => {
      const createdBy = userNames[po.created_by_email] || po.created_by_email || "-";

      return [
        po.po_number || "-",
        "-", // PR Number (not directly available in Po object)
        po.supplier_name || "-",
        po.delivery_partner || "-",
  ...
  const statusCounts = {
    ...
    shipment_at_supplier: safePos.filter(
      (p) => p.status === "shipment_at_supplier"
    ).length,
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-3 align-top text-gray-700 text-xs">
                        {userNames[po.created_by_email] || po.created_by_email?.split("@")[0] || "-"}
                      </td>
