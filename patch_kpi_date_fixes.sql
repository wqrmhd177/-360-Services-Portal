    ...
    SELECT
      ...
      CASE
        ...
        WHEN p_group_id = 'shipped' THEN
          CASE WHEN p.shipment_date IS NOT NULL
            THEN GREATEST(0, v_today - (p.shipment_date AT TIME ZONE 'America/Los_Angeles')::DATE)
            ELSE NULL
          END
...
COMMENT ON FUNCTION get_ops_orders_status_detail IS
  'KPI drill-down: aging date per group — confirmationDate, approvedDate, shipmentDate, undeliveredDate, orderDate.';
