...
export default function PODownloadButton({ po, prNumber }: PODownloadButtonProps) {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      doc.text(`Date: ${po.created_at ? new Date(po.created_at).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : "-"}`, margin, y);
      y += 5;
      doc.text(`Status: ${po.status.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}`, margin, y);
      y += 5;
      ...
      autoTable(doc, {
        startY: y,
        head: [["#", "Product Name", "SKU Code", "Qty", "Rate (AED)", "Amount (AED)"]],
        body: tableRows,
        theme: "striped",
      ...
      // Remarks
      if (po.remarks) {
        const remarksY = finalY + 16;
        doc.setFontSize(9);
        doc.setFont("helvetica", "bold");
        doc.text("Remarks:", margin, remarksY);
        doc.setFont("helvetica", "normal");
        const lines = doc.splitTextToSize(po.remarks, pageWidth - margin * 2 - 20);
      ...
      doc.text("Generated by 360 Services Portal", margin, pageHeight - 10);
